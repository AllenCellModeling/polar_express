...
def compute_voxel_matrix(scale_factors, centroid, masked_channels):
    ...
    angles = [
        np.arccos(
            np.dot(unit_z_vector, cyto_indices[i, :] - centroid)
            / (
                np.dot(cyto_indices[i, :] - centroid, cyto_indices[i, :] - centroid)
                ** 0.5
            )
        )
    ...
    nucleus_dists = [
        np.min(
            distance.cdist(
                np.reshape(cyto_indices[i, :], (1, 3)),
                nucleus_shell_indices,
                "euclidean",
            )
        )
        for i in range(num_voxels)
    ...
    mem_dists = [
        np.min(
            distance.cdist(
                np.reshape(cyto_indices[i, :], (1, 3)), mem_shell_indices, "euclidean"
            )
        )
        for i in range(num_voxels)
    ]
